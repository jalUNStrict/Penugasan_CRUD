...
@router.get("/", response_model=EventResponse, dependencies=[Depends(get_current_user)]) 
def get_events(service: EventService = Depends(get_event_service)):
    events = service.list_events() 
    return {"code": 200, "message": "Events retrieved successfully", "data": events}
...
@router.put("/{event_id}", dependencies=[Depends(admin_only)])
