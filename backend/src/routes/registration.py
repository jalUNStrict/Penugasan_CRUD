...
@router.get("/", response_model=list[RegistrationResponse], dependencies=[Depends(get_current_user)])
def get_registrations(ctrl: RegistrationController = Depends(RegistrationController)):
    return ctrl.list_registrations()

@router.post("/", response_model=RegistrationResponse, dependencies=[Depends(get_current_user)])
...
@router.put("/{id}", response_model=RegistrationResponse, dependencies=[Depends(get_current_user)])
